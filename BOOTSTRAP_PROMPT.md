...
- 예: Next.js 라우트 `/api/<id>` schema, Android intent-filter MAIN+HOME+DEFAULT+LAUNCHER 같은 것
...
{{PROJECT_TITLE}}   → 표시 이름 (예: "SampleApp Launcher")
...
- 슬러그: 디렉토리 이름의 의미있는 짧은 부분 (예: `sampleapp_launcher` → `sampleapp`)
