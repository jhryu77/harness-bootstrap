...
> 배포물에서 레퍼런스 프로젝트 도메인 용어 잔여분을 제거하고, 재발을 CI 로 막는다.

### Fixed

- **레퍼런스 도메인 용어 잔여분 17곳 제거** (7파일) - `ccb5a7c` 에서 예제 스냅샷은 중립 재작성됐으나 **산문 서술**에 원본 제품 용어가 남아 있었다. 진화 곡선 내러티브(`01_philosophy.md` §원칙 9 / `OVERVIEW.md`), `examples/case_study.md` 컨텍스트, BRAIN 섹션 표(`04_ssot_brain_state.md`), `dev_PROJECT.md.template` 헤더가 대상. 파일명·슬래시명은 find-replace 로 잡히지만 산문은 그물을 빠져나간다. 부수 효과로 `OVERVIEW.md` 가 예제 앱을 스냅샷 `CLAUDE.md` 와 **다른 제품으로 소개하던 모순**도 해소.

### Added

- **`scripts/check_domain_terms.py`** - 도메인 용어 유출 검사기. git 추적 파일 중 텍스트 확장자만 스캔하고 자기 자신과 `plans/` 는 제외한다. 일반 단어(`분할` / `Picker` / `런처` 단독)는 목록에 넣지 않는다 - 오탐이 나면 검사가 무시당하고 그게 검사 없는 것보다 나쁘다. 정당한 사용은 그 줄에 `domain-terms:allow` 주석으로 통과시킨다. 인자 없이 저장소 전체, 인자로 특정 파일만 검사.
- **`.github/workflows/domain-terms.yml`** - push(main) / PR / 수동 실행 시 위 검사기 구동. 로컬 git hook 은 clone 시 따라가지 않으므로 CI 에서 강제한다.
