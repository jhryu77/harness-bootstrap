...
```
sampleapp/                                     (예: /Users/<user>/workspace/sampleapp)
...
| UI | View 시스템 + RecyclerView(ListAdapter) + Fragment + ViewBinding |
| 도메인 | 일반 Android 앱 (MAIN+LAUNCHER 진입, 로컬 우선 리스트-디테일) |
| 리스트 | 페이지네이션 20개/페이지, 1~100 클램프, Room + Repository |
| 저장 | Room DB(`items`) + SharedPreferences(`settings`/`sync_state` 분리) |
| 업데이트 | OkHttp 버전 체크 + `FileProvider` install 인텐트 (INTERNET, 네트워크 fail-soft 의무) |
| 외부 의존 | 외부 SSOT/sibling 프로젝트/원격 API **없음** (자기 완결. OTA 는 Supabase Storage) |
...
FullScaleApp 의 디자인 토큰 / 반응형 / 보안 / Pose / RPC 검사 항목은 도메인 무관으로 모두 제거됨.
...
| `/dev_sampleapp` | 메인 세션이 코드 구현. 5섹션 도메인 가이드 (진입 / 리스트 / 상세·편집 / 저장 / 업데이트) |
...
| `/test_sampleapp` | 수동 검증 체크리스트 (Build / 앱 진입 / 리스트 / 저장 / fail-soft) |
...
### TC: 앱 진입
- [ ] MAIN / LAUNCHER intent-filter 정상 (런처에서 아이콘 실행)
- [ ] configChanges / 회전 시 상태 보존

### TC: 리스트 / 페이지네이션
- [ ] 초기 20개 로드 / 1~100 범위 클램프 (`coerceIn(1, MAX_PAGE_SIZE)`)
- [ ] 스크롤 끝 도달 시 다음 페이지 Room 쿼리 + DiffUtil 갱신

### TC: 저장 - Room / Prefs
- [ ] 항목 추가/편집 → Room `items` 영구화 → 재시작 복원
- [ ] `SortOrder`/설정은 SharedPreferences(`settings`) 영구화
- [ ] Room `@Database(version)` 증분 시 `Migration` 동반 (데이터 손실 0)

### TC: 자가 업데이트 fail-soft
- [ ] 네트워크 없음/타임아웃 시 예외 흡수, 크래시 0건
- [ ] `onUpdateCheckFailed` Toast 안내
- [ ] OkHttp / install 인텐트 호출 모두 try-catch
...
7. **앱 진입 비타협** - `MAIN`+`LAUNCHER` intent-filter + packageName 보존
8. **리스트 상수 비타협** - `PAGE_SIZE 20 / MAX_PAGE_SIZE 100` 상수 + `coerceIn` 클램프
9. **저장 호환성** - Room entity 컬럼명·`SortOrder.storageKey` 변경 = 마이그레이션 필수, enum 순서 변경 금지
10. **Room 스키마 버전** - `@Database(version=N)` 증분 시 `Migration` 동반, dp/절대 ms 혼용 금지
11. **네트워크 fail-soft** - OkHttp / OTA 체크 / install 인텐트 호출 모두 try-catch 의무
