...
| AC-2 | `grep -c "coerceIn(1, MAX_PAGE_SIZE" ItemRepository.kt` | 출력 ≥ 1 |
...
- `python3 -m py_compile ci_gate_<project>.py` 통과
...
사용자: Dashboard 업로드 / Vercel deploy / DB row 활성화 등 외부 액션
       ↓
/eval_agent_<release>        Storage 확인 + 토글 + 단말 logcat + 앱 재진입 검증
       ↓
사용자 확정
       ↓
...
`<project>` 자리에 들어갈 후보:
- Android client: `sampleapp` / `app` / `client`
- Next.js admin: `admin` / `webapp` / `server`
- iOS app: `ios`

본래 프로젝트의 통념 명칭을 따른다. 다만 **너무 짧으면 (예: `app`) 혼란**, **너무 길면 (예: `sampleapp_app_main`) 타이핑 부담**. 보통 6~10자 추천.
...
| 비타협 항목 점검 | **강한** (진입 / 저장 등) | **강한** (hook / 권한) | **매우 강한** (versionCode / 시그너처) | **강한** (RLS / 인증) |
