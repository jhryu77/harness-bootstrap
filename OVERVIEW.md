...
4. /dev_<project>
   ──────────────────▶  plan.md 읽고 Edit/Write
                          ...
                          ├── 매 Edit/Write 후
                          │   ┌─── PostToolUse hook ───┐
                          │   │  python3 ci_gate_*.py    │ → stderr 위반 시
...
| plan 영향 평가 | 비타협 항목 (진입점 / 스키마 / 인증) | 하네스 무결성 (hook / frontmatter / 권한 / archive) |
...
| | `statusline.py` | (필수) 전역 상태바 - CW/5H/7D 사용률 + reset 카운트다운. 치환 불필요, `~/.claude/` 복사 |
...
| 디렉토리/파일 | 무엇 |
|---|---|
| [`examples/sampleapp_snapshot/`](./examples/sampleapp_snapshot/) (23 파일) | sampleapp_app (Android 리스트-디테일 앱) **하네스 풀 구성 예시** |
...
| [`examples/case_study.md`](./examples/case_study.md) ★ | **두 사례 비교 종단 시나리오** - sampleapp_app Phase Z 의 10단계 흐름 + sampleapp_server 적용 가이드 |
...
- **sampleapp_app** (Android 앱) - OTA 배포 / 크래시 보고 / 진단 수집까지 포함한 풀 스택 구성 예시
