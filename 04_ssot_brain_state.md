...
| **9. 스키마 / 진입점 / 인증 등 영역별 비타협** | 매직 넘버 / 검증 함수 / 필터 조건 |
...
`/sync_brain` 슬래시 1개로 메인 세션이 직접 갱신. (컨텍스트 격리나 모델 티어링을 sync 까지 걸고 싶으면 **선택적으로** 디스패처+서브에이전트로 둘 수 있다 - 05_subagent_design.md "선택 패턴 - sync_brain 을 서브에이전트로". 단일 프로젝트 기본값은 이 커맨드다.)
