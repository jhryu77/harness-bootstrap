...
서브에이전트(plan/eval)만 티어링한다. dev(메인세션 멀티턴)는 커맨드 frontmatter 가 턴 한정이라
커버 안 되므로, --with-session-baseline 을 주면 settings.local.json 의 model 도 세션 baseline 으로
설정해 dev 루프까지 티어링한다 (05_subagent_design.md "티어링 갭" 참조).

사용법:
  python3 .agent/scripts/model_policy.py set <high|medium|low|inherit>
  python3 .agent/scripts/model_policy.py set <tier> --with-session-baseline   # dev 루프까지
  python3 .agent/scripts/model_policy.py show
"""
...
# dev(메인세션) 루프 세션 baseline. 위 MODEL_POLICY 는 **서브에이전트만** 티어링하는데,
# 정작 토큰을 가장 많이 쓰는 dev 는 서브에이전트가 아니라 메인세션 멀티턴이라 커버되지 않는다.
# 커맨드 frontmatter model 은 "호출한 그 턴만" 유효(공식 문서) → dev 루프를 티어링하려면
# settings.local.json 의 model 을 세션 baseline 으로 설정해야 한다 (05_subagent_design.md "티어링 갭" 참조).
# --with-session-baseline 플래그로만 적용된다 (기본은 서브에이전트 frontmatter 만 건드림).
SESSION_BASELINE = {"high": "opus", "medium": "sonnet", "low": "sonnet", "inherit": None}
...
# ---------------------------------------------------------------- session baseline (dev 루프)

def update_settings_model(root: pathlib.Path, model) -> str:
    """settings.local.json 의 model 키만 read-modify-write. 나머지 키(permissions/hooks 등)는 보존.
    model 이 None(inherit) 이면 model 키를 제거한다 (세션이 기본 모델로 돌게).
    반환: 사람이 읽을 결과 문자열."""
    path = root / ".claude" / "settings.local.json"
    if not path.is_file():
        return f"skipped: {path} 없음"
    try:
        with open(path, encoding="utf-8") as f:
            data = json.load(f)
    except (json.JSONDecodeError, OSError) as e:
        return f"skipped: 파싱 실패({e})"
    ...
    with open(path, "w", encoding="utf-8", newline="") as f:
        ...
        f.write("\n")
    return f"session baseline: {before} -> {model}  (/model 실행 또는 재시작 후 반영)"
...
def cmd_set(args) -> int:
    ...
    # dev(메인세션) 루프 세션 baseline - 옵션. 안 주면 dev 는 항상 세션 모델(대개 풀 모델)로 돈다.
    if getattr(args, "with_session_baseline", False):
        print("  " + update_settings_model(root, SESSION_BASELINE[tier]))
    else:
        print("  (dev 루프 세션 baseline 미설정 - 서브에이전트만 티어링됨. "
              "dev 까지 티어링하려면 --with-session-baseline)")
...
def main() -> int:
    ...
    p_set.add_argument("--with-session-baseline", action="store_true",
                       help="settings.local.json 의 model 도 세션 baseline 으로 설정 (dev 메인세션 루프 티어링)")
